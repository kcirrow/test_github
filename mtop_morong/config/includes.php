...
if ($page == "login") {
	...
	if (!empty($_POST['scode'])) {
		echo $object->getdetStatus($_POST['scode']);
	}
...
}else if ($page == "getopmotor1") {
	if (!empty($_POST['code'])) {
		echo $object->getopMotor1($_POST['code']);
	}
...
// 		}
	...
	if (!empty($_POST['opcode']) && !empty($_POST['motorid']) && !empty($_POST['drivercode']) && !empty($_POST['yr'])) {
...
// 	if (!empty($_POST['mopcode']) && !empty($_POST['mtoda']) && !empty($_POST['mmake']) && !empty($_POST['mengine']) && !empty($_POST['mchassis']) && !empty($_POST['myrmodel']) && !empty($_POST['mcolor']) && !empty($_POST['mcert']) && !empty($_POST['mdtissue']) && !empty($_POST['mplate']) && !empty($_POST['magency']) && !empty($_POST['mremarks']) && !empty($_POST['mplatecolor']) && !empty($_POST['mmvno']) && !empty($_POST['morcrname'])) {
	...
	if (!empty($_POST['mid']) && !empty($_POST['mopcode']) && !empty($_POST['mtoda']) && !empty($_POST['mmake']) && !empty($_POST['mengine']) && !empty($_POST['mchassis']) && !empty($_POST['mcert']) && !empty($_POST['mplate']) && !empty($_POST['mmvno']) && !empty($_POST['morcr']) && !empty($_POST['morcrdate']) && !empty($_POST['mexpired']))  {
		echo $object->updateMotor($_POST['mid'], $_POST['mopcode'], "", $_POST['mtoda'], $_POST['mmake'], $_POST['mengine'], $_POST['mchassis'], $_POST['myrmodel'], $_POST['mcolor'], $_POST['mcert'], $_POST['mdtissue'], $_POST['mplate'], $_POST['magency'], $_POST['mremarks'], $_POST['mplatecolor'], $_POST['mmvno'], (isset($_POST['inspect-stoplight-switch']) ? 'on' : 'off'), $_POST['morcrname'], $_POST['morcr'], $_POST['morcrdate'], $_POST['mmunplateno'], $_POST['mmtop'], $_POST['mexpired']);
	} else {
...
}else if ($page == "getchangedriverreport") {
	if (!empty($_POST['from']) && !empty($_POST['to'])) {
		echo $object->getChangeDriverReport($_POST['toda'], $_POST['from'], $_POST['to'], $_POST['year']);
	} else {
		echo json_encode(array("result" => false, "msg" => 'Select a TODA in order to display the report.', "data" => []));
	}
	...
	if (!empty($_POST['trcode']) && !empty($_POST['reason']) && !empty($_POST['auth'])) {
		echo $object->cancelDropping($_POST['trcode'], $_POST['reason'], $_POST['auth']);
	} else {
		echo json_encode(array("result" => false, "msg" => 'Empty field detected.'));
	}
	...
	//if (!empty($_POST['stags']) && !empty($_POST['stagscode']) && !empty($_POST['strcode'])) {
    if (!empty($_POST['stags']) && !empty($_POST['strcode'])) {
        echo $object->updateStatus($_POST['stags'], $_POST['strcode']);
    }else {
		echo json_encode(array("result" => false, "msg" => 'Empty field detected.'));
	}
