...
class Myclass extends Dbh
{
	...
	public function dashboardcount() {
	  $stmt = $this->connect()->prepare("SELECT count(*) as fran FROM tbl_application WHERE iscancel = 0 AND appl_status = 'NEW' AND yr = ?");
		...
		$data = array("newrenew" => $newrenew["fran"], "renew" => $renew["franr"], "cm" => $cm["cm"], "co" => $co["co"], "drop" => $drop["cdrop"],"totamt" => $totamt["totamt"]? number_format(str_replace(",", "", $totamt['totamt']), 2) : '0.00');
...
public function getStatus()
	{
		...
		$stmt = $this->connect()->query("SELECT trcode, humanpin, motorcode, first_name, middle_name, last_name, ext_name, address_house_no, address_street_name, address_subdivision, address_brgy, address_municipality, address_province, a.franchise_no, appl_status, Tags, dttm, target_path, make FROM tbl_application AS a LEFT JOIN tbl_humans AS b ON a.opercode = b.humanpin LEFT JOIN tbl_motor AS c ON a.motorcode = c.motorpin  WHERE humantype = 0");
...
//carlo
	...
	{
		$stmt = $this->connect()->prepare("SELECT trcode, opercode,drivercode, motorcode, franchise_no, appl_status, Tags,tagscode, dttm FROM tbl_application where trcode = ?");
		
		//$stmt = $this->connect()->prepare("SELECT trcode, humanpin, motorcode, CONCAT(first_name,' ', LEFT(middle_name, 1), '. ', last_name, ' ', ext_name) as fullname, CONCAT(address_house_no, ' ', address_street_name, ' ', address_subdivision, ' ', address_brgy, ' ', address_municipality, ', ', address_province, ', ', address_region) as addr, a.franchise_no, appl_status, Tags,tagscode, dttm, target_path, make FROM tbl_application AS a LEFT JOIN tbl_humans AS b ON a.opercode = b.humanpin LEFT JOIN tbl_motor AS c ON a.motorcode = c.motorpin  WHERE humantype = 0 AND humanpin = ?");
		$stmt->bindValue(1, $code);
		$stmt->execute();
		return json_encode($stmt->fetch(PDO::FETCH_ASSOC));
	}
	...
	{
		$stmt = $this->connect()->prepare("SELECT motorpin, opercode, bodyno, toda, franchiseno, make, status, engine, chassis, yearmodel, color, foryear, crno, mvno, crdate, plateno, ltobranch, remarks, platecolor, crname, crswitch, orcr, orcrdate, munplateno FROM tbl_motor WHERE motorpin = ?");
...
public function updateStatus($tags, $sid) {
	    try {
	        $stmt = $this->connect()->prepare("UPDATE tbl_application SET tags = ? WHERE id = ?");
    	    $stmt->bindValue(1, $tags);
	...
	public function submitChangeMotor($opcode, $opmotor, $newmotor, $yr, $drcode, $remarks, $newopcode)
	{
		...
		// CHANGE MOTOR SAVING
		$stmt = $this->connect()->prepare("INSERT INTO tbl_changemotor (trcode, yr, opercode, motorcode, newopercode, newmotorcode, dateapplication, tagscode, Tags, encoded_by, remarks) VALUES (:trcode, :yr, :opercode, :motorcode, :newopercode, :newmotorcode, :dateapplication, 2, 'FOR INSPECTION', :encoded_by, :remarks)");
		$stmt->bindParam(":trcode", $pin);
	...
	public function getAssFees($applstatus, $trcode)
	{
		try {
			...
			if ($applstatus == "RENEW") {
				...
				$dtreg = ($settings['expmode'] == 0 ? ($data['dtexprd'] != "1990-01-01" ? date_create($data['dtexprd']) : $dtnow) : date_create($data['foryear']."-01-01"));
    ...
    // 		}
        ...
        $stmt->execute();
			...
			case "CHANGE MOTOR":
				...
				// MOTOR UPDATE
				$stmt2 = $this->connect()->prepare("UPDATE tbl_motor AS a JOIN tbl_changemotor AS b ON a.motorpin = b.newmotorcode SET a.bodyno = a.franchiseno, last_renew = :franchise_date, last_renew_sticker = :franchise_date, foryear = year(now()), a.dtexprd = :dtexprd, a.dtexprdstick = :dtexprd, orfran = b.orno, orstick = b.orno WHERE b.trcode = :trcode");
...
// 			}
	...
	public function DeleteMotor($opcode, $reason, $auth)
	{
		...
		if ($stmt->execute()) {
			...
			$stmt->execute();
			$data = $stmt->fetch(PDO::FETCH_ASSOC);
			$this->saveLogs("DELETE MOTOR", json_encode(array($pdate)), json_encode(array($data)), json_encode(array("humanpin" => $pin, "reason" => $reason)), $_SESSION['username'], "MOTOR");
...
//         }
	...
	public function updateMotor($mid, $opcode, $body, $toda, $make, $engine, $chassis, $yrmodel, $color, $cert, $dtissue, $plate, $agency, $remarks, $platecolor, $mvno, $orcrswitch, $orcrname, $orcr, $orcrdate, $munplate, $franchiseno)
	{
		if ($body != "") {
	      $stmt = $this->connect()->prepare("SELECT * FROM tbl_motor WHERE toda = ? AND CONVERT(bodyno,UNSIGNED INTEGER) = ? AND status = 'AVAILABLE'");
    		$stmt->bindValue(1, $toda);
    ...
    $stmt = $this->connect()->prepare("SELECT * FROM tbl_motor WHERE motorpin = ?");
		...
		$stmt = $this->connect()->prepare("UPDATE tbl_motor SET opercode = :opercode, toda = :toda, make = :make, engine = :engine, chassis = :chassis, yearmodel = :yearmodel, color = :color, crno = :crno, crdate = :crdate, plateno = :plateno, ltobranch = :ltobranch, remarks = :remarks, platecolor = :platecolor, mvno = :mvno, crname = :crname, crswitch = :crswitch, orcr = :orcr, orcrdate = :orcrdate, munplateno = :munplateno, franchiseno = :franchiseno WHERE motorpin = :motorpin");
	...
	public function saveToda($code, $route, $remarks, $toda, $pres, $contactno)
	{
		...
		try {
			...
			try {
				$dbh->beginTransaction();
				$stmt->execute(array($code, $route, $remarks,$pre,$contactno));
				$lid = $dbh->lastInsertId();
				$dbh->commit();
	...
	{
    	$stmt = $this->connect()->prepare("SELECT IF(make <> 'E-TRIKE', LPAD(bodyno, 4,'0'), bodyno) AS bodyno,trcode, last_name, middle_name, first_name, addr, make, chassis, engine, plateno, franchise_date,IF(make <> 'E-TRIKE', LPAD(fno, 4,'0'), fno) AS fno, last_renew, toda, appl_status FROM (SELECT bodyno, ifnull(last_name,'N/A') as last_name, ifnull(first_name,'N/A') as first_name, ifnull(middle_name,'N/A') as middle_name, ifnull(CONCAT(address_house_no, ' ', address_street_name, ' ', address_subdivision, ' ', address_brgy, ' ', address_municipality, ', ', address_province),'N/A') as addr, make,chassis,engine,plateno, a.franchise_date,a.franchise_no as fno,a.yr as last_renew,toda,appl_status, trcode FROM tbl_application AS a LEFT JOIN tbl_motor AS b ON a.motorcode = b.motorpin LEFT JOIN tbl_humans AS c ON a.opercode = c.humanpin WHERE toda LIKE ? AND (franchise_date BETWEEN ? AND ?) AND yr = ? AND a.franchise_no <> '' AND appl_status <> 'TDP' AND iscancel = 0 ORDER BY bodyno asc) as z GROUP BY trcode ORDER BY bodyno asc ");
		$stmt->bindValue(1, "%".$toda."%");
	    ...
	    try {
            ...
            switch ($data['appl_status']) {
    			...
    			case "CHANGE OWNERSHIP":
    				...
    				break;
    			case "DROPPING":
    				return $this->getdetDropping($trcode);
    				break;
    			default:
    				return json_encode(array("result" => false, "msg" => "Something went wrong can't load the data."));
    		}
