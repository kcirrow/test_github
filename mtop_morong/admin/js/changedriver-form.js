...
if (getUrlParameter("trcode") !== undefined) {
  $.ajax({
    url: "../config/includes.php?var=getdetchangedriver",
    method: "POST",
    data: {
    ...
    success: function (e) {
      if (e) {
        state = e;
        $("#form-operatorid").html("<b>Driver ID: </b>" + e.opcode);
        ...
        $("#form-operatorid-new").html(
          "<b>Driver ID: </b>" + e.newoper.opcode
        );
  ...
  $.ajax({
    url: "../config/includes.php?var=getdetcminitialmotors",
    method: "POST",
    data: {
    ...
    success: function (e) {
      if (e.result) {
        state = e.data;
        $("#form-operatorid").html("<b>Driver ID: </b>" + e.data.opcode);
        ...
        $("#form-applicationtype").val("CHANGE DRIVER");
      } else {
          Swal.fire({
              icon: "warning",
              text: e.msg,
              ...
              if (result.isConfirmed) {
                window.location.href =
                  baseurl + "/admin/changedriver-table.php";
              }
            });
      }
    },
...
$("#operatortable").on("dblclick", "tr", function () {
  ...
  $("#form-operatorid-new").html("<b>Driver ID: </b>" + d.humanpin);
...
$("#operatortable").on("click", "#btn-operator-select", function () {
  ...
  $.ajax({
    ...
    success: function (e) {
      if (e.result) {
        ...
        btype = "";
        $("#form-operatorid-new").html("<b>Driver ID: </b>" + state["newopcode"]);
  ...
  $.ajax({
    ...
    success: function (e) {
      if (e.result) {
        state["motorid"] = e.motorpin;
        $("#mid").html(e.motorpin);
        $("#form-motorid-new").html(e.motorpin);
...
$("#btn-motor-edit").click(function () {
  $.ajax({
    ...
    success: function (e) {
      if (e.result) {
        Swal.fire("Nice!", e.msg, "success");
        $("#form-motorid-new").html(state.motorid);
        $("#form-motortoda-new").html(
          $("#mtoda").val() + " - " + $("#mbody").val()
        );
        $("#form-motormake-new").html($("#mmake").val());
...
$("#btn-motor-select").click(function () {
  var d = motortable.row($("#motortable .selected").closest('tr')).data();
  state["motorid-new"] = d.motorpin;
  state["opcode-new"] = d.ophumanpin;
  $("#form-motorid-new").html(d.motorpin);
  $("#form-motortoda-new").html(d.todabody);
    ...
    success: function (e) {
      if (e.result) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            window.location.href =
              baseurl + "/admin/changedriver-form.php?trcode=" + e.trcode;
          }
        });
      } else {
        Swal.fire("Ops!", e.msg, "error");
...
$("#btn-upload-pictures").click(function () {
  var data1 = new FormData($("#repapplication-forms")[0]);
