...
$("#settingstodatable tbody").on("click", "#btn-settings-toda-view",function () {
	...
	$.ajax({
		...
		success: function (e) {
			...
			$("#tpres").val(e.data.todaPres);
			$("#tcontactno").val(e.data.contactno);
			$("#ttest").val(e.data.testcol);
			$("#div-settings-toda-table, #btn-toda-save").hide();
			$("#div-settings-toda-form, #btn-toda-edit").show();
		}
	});
});
