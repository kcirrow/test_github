$(document).ready(function () {
  $("#changeownershiptable").on("click", "#btn-view", function () {
    var d = changeownershiptable.row($(this).closest("tr")).data();
    window.location.href =
      baseurl + "/admin/changeownership-form.php?trcode=" + d.trcode;
  });
  var cmselection = "";
  $("#btn-apply-changeownership").click(function () {
  ...
  $("#changeownership-table").addClass("active");

  $("#changeownershiptable").on("click", "#btn-cancel", function () {
    var d = changeownershiptable.row($(this).closest("tr")).data();
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: "../config/includes.php?var=cancelChangeOwnership",
              method: "POST",
              data: {
                trcode: d.trcode,
                reason: $("#reason").val(),
...
$("#btn-cmselection-select").click(function () {
    ...
    window.location.href =
      "<?php echo $class->base_url(); ?>" + "/admin/changeownership-form.php?motor=" + d.motorpin;
});

//=============================================================================================================
// Assessment Section
//=============================================================================================================
changeownershiptable.on(
  "click",
  "#btn-changeownership-assessment",
  function () {
    var d = changeownershiptable.row($(this).closest("tr")).data();
    assyes = changeownershiptable;
...
changeownershiptable.on(
  "click",
  "#btn-changeownership-inspection",
  function () {
    var d = changeownershiptable.row($(this).closest("tr")).data();
    state = d;
    $.ajax({
...
// Releasing Section
//=============================================================================================================
changeownershiptable.on("click", "#btn-changeownership-release", function () {
  var d = changeownershiptable.row($(this).closest("tr")).data();
  state = d;
  if (d.target_path == "") {
