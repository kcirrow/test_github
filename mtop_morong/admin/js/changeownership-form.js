...
if (getUrlParameter("trcode") !== undefined) {
  $.ajax({
    url: "../config/includes.php?var=getdetchangeownership",
    method: "POST",
    data: {
  ...
  $.ajax({
    ...
    success: function (e) {
      if (e.result) {
        ...
        $("#form-applicationtype").val("CHANGE OWNERSHIP");
      } else {
          Swal.fire({
              icon: "warning",
              text: e.msg,
              ...
              if (result.isConfirmed) {
                window.location.href =
                  baseurl + "/admin/changeownership-table.php";
              }
            });
      }
    },
...
$("#btn-submit-franchise").click(function () {
  ...
  $.ajax({
    url: "../config/includes.php?var=submitchangeownership",
    method: "POST",
    data: state,
    dataType: "JSON",
    success: function (e) {
      if (e.result) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            window.location.href =
              baseurl + "/admin/changeownership-form.php?trcode=" + e.trcode;
          }
        });
      } else {
        Swal.fire("Ops!", e.msg, "error");
