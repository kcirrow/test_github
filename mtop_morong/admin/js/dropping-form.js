  ...
  $.ajax({
    ...
    success: function (e) {
      if (e) {
        ...
        $("#form-reason").val(e.reason);
        $("#form-trcode-new").html(e.trcode);

        $("#form-applicationyear").attr("disabled", true);
        $("#form-reason").attr("disabled", true);
        $("#form-remarks-new").attr("disabled", true);
        $("#btn-motor-operator").remove();
        $("#btn-motor-edit").remove();
  ...
  $.ajax({
    ...
    success: function (e) {
      if (e.result) {
        ...
        $("#form-lastrenew").html(e.last_renew);
        $("#form-trcode").html(e.trcode);
        $("#form-applicationtype").val("DROPPING");
      } else {
          Swal.fire({
              icon: "warning",
              text: e.msg,
              ...
              if (result.isConfirmed) {
                window.location.href =
                  baseurl + "/admin/dropping-table.php";
              }
            });
      }
    },
  ...
  $.ajax({
    ...
    success: function (e) {
      if (e.result) {
        ...
        $("#form-operatorid").html("<b>Operator ID: </b>" + e.ophumanpin);
    ...
    success: function (e) {
      if (e.result) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            window.location.href =
              baseurl + "/admin/dropping-form.php?trcode=" + e.trcode;
          }
        });
      } else {
        Swal.fire("Ops!", e.msg, "error");
